...
if( function_exists( 'get_the_subtitle' ) ) :
	 ...
	 */
	if( ! function_exists( 'siftscience_subtitles_remove_default' ) ) :
	function siftscience_subtitles_remove_default( $subtitle_view_supported ){
	...
	add_filter( 'subtitle_view_supported', 'siftscience_subtitles_remove_default' );
	 ...
	 */
	if( ! function_exists( 'siftscience_subtitles_implementation' ) ) :
	function siftscience_subtitles_implementation( $the_categories, $post_id ){
	...
	add_filter( 'siftscience_entry_subtitle', 'siftscience_subtitles_implementation', 10, 2 );
	 ...
	 */
	if( ! function_exists( 'siftscience_subtitles_posted_on' ) ) :
	function siftscience_subtitles_posted_on( $posted_on, $post_id ){
		...
		if( $subtitle && is_singular() ){
			...
			$entry_subtitle = siftscience_get_entry_subtitle( $post_id );

			if( $entry_subtitle ){

				return sprintf( __( '%s under <span class="taxonomies">%s</span>', 'siftscience' ), $posted_on, $entry_subtitle );
	...
	add_filter( 'siftscience_posted_on', 'siftscience_subtitles_posted_on', 10, 2 );
